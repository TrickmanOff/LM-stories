...
class GDriveStorage(ExternalStorage):
    ...
    def _import_encoder(self, experiments_storage: ExperimentsStorage, encoder_name: str) -> None:
        local_encoder_dirpath = experiments_storage.get_encoder_dirpath(encoder_name)
        gdrive_encoders_dir_id = self._get_encoders_dir()
        self._import_archive(gdrive_encoders_dir_id, encoder_name, local_encoder_dirpath)

    def _export_encoder(self, experiments_storage: ExperimentsStorage, encoder_name: str) -> None:
        local_encoder_dirpath = experiments_storage.get_encoder_dirpath(encoder_name)
        gdrive_encoders_dir_id = self._get_encoders_dir()
        self._export_as_archive(gdrive_encoders_dir_id, encoder_name, local_encoder_dirpath)
    ...
    def _get_encoders_dir(self) -> str:
        """
        Creates if it does not exist
        :return: id
        """
        return self._get_subdir(parent_dir_id=self.storage_dir_id, subdir_name=self.ENCODERS_DIR)
    ...
    def _export_as_archive(self, dir_id: str, archive_name: str, from_path: Union[str, Path]) -> None:
        """
        :param dir_id: directory to upload the archive
        :param archive_name: the name of the uploaded archive (without extension)
        :param from_path: the local directory to archive its content
        """
        with tempfile.TemporaryDirectory() as archive_dir:
    ...
    def _import_archive(self, dir_id: str, archive_name: str, to_dirpath: Union[str, Path]) -> None:
        """
        The contents of the archive will be moved to `to_dirpath`
        """
        with tempfile.TemporaryDirectory() as tmp_dir:
            query = f'"{dir_id}" in parents and title="{archive_name}" and trashed=false'
            archive_file = self.drive.ListFile({'q': query}).GetList()[0]
            downloaded_archive_filepath = os.path.join(tmp_dir, f'{archive_name}.{ARCHIVE_FORMAT}')
            archive_file.GetContentFile(downloaded_archive_filepath)
            shutil.unpack_archive(downloaded_archive_filepath, to_dirpath, ARCHIVE_FORMAT)
    ...
    def get_available_encoders(self) -> List[str]:
        encoders_dir = self._get_encoders_dir()
        encoders_list = self.drive.ListFile(
            {'q': f"'{encoders_dir}' in parents and trashed=false"}).GetList()
        return [encoder['title'] for encoder in encoders_list]

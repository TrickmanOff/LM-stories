...
so one should not work with these paths in other way in the code
"""
import datetime
import os
import logging
...
class ExperimentsStorage:
    def __init__(self, experiments_dir: Union[str, Path] = 'saved/models',
                 encoders_dir: Union[str, Path] = 'saved/encoders'):
        self._experiments_dir = Path(experiments_dir)
        self._encoders_dir = Path(encoders_dir)

    def get_run(self, exp_name: str, run_name: Optional[str] = None, create_run_if_no: bool = False) -> RunStorage:
        if run_name is None:
            run_name = datetime.datetime.now().strftime(r"%m%d_%H%M%S")
    ...
    def get_encoder_dirpath(self, encoder_name: str) -> Path:
        encoder_dirpath = self._encoders_dir / encoder_name.lower().replace(' ', '_')
        encoder_dirpath.mkdir(parents=True, exist_ok=True)
        return encoder_dirpath
